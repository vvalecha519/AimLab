...
namespace Unity.FPS.Gameplay
{
    ...
    public class PlayerCharacterController : MonoBehaviour
    {
        ...
        void HandleCharacterMovement()
        {
            ...
            m_CameraHorizontalAngle += m_InputHandler.GetLookInputsHorizontal() * RotationSpeed * RotationMultiplier;
            m_CameraVerticalAngle += m_InputHandler.GetLookInputsVertical() * RotationSpeed * RotationMultiplier;
            // limit the camera's vertical angle to min/max
            m_CameraVerticalAngle = Mathf.Clamp(m_CameraVerticalAngle, -89f, 89f);
            m_CameraHorizontalAngle = Mathf.Clamp(m_CameraHorizontalAngle, -80f, 80f);
            // apply the vertical angle as a local rotation to the camera transform along its right axis (makes it pivot up and down)
            PlayerCamera.transform.localEulerAngles = new Vector3(m_CameraVerticalAngle, m_CameraHorizontalAngle, 0);
